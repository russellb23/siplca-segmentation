...
class PLCA():
    ...
    def analyze(cls, V, rank, niter=100, convergence_thresh=1e-9,
        ...
        logprob : float
        """
        # Normalizing the magnitudes - V
        norm = V.sum()
        V /= norm
        # cls is used for class methods whereas self is used for instance methods
        params = cls(V, rank, **kwargs) # params is a class instance here
        # Initializing the params class
        iW, iZ, iH = params.initialize()
        # Assigning constraints/priors for W, Z and H
        ...
        for n in range(niter):
            ...
            if n % printiter == 0:
                ...
                print('Iteration {}: logprob = {}'.format(n, logprob))
            if plotiter and n % plotiter == 0:
                params.plot(V, W, Z, H, n)
                if not plotfilename is None:
                    plt.savefig('%s_%04d.png' % (plotfilename, n))
            if logprob < oldlogprob:
                print('Warning: logprob decreased from {} to {} at iteration '
                             '{}!'.format(oldlogprob, logprob, n))
    ...
    def do_estep(self, W, Z, H):
        ...
        for z in range(self.rank):
            # Compute the outer products * divergence
            tmp = np.outer(W[:,z] * Z[z], H[z,:]) * VdivWZH
            self.VRW[:,z] = tmp.sum(1) # Sum of column array elements
            self.VRH[:,z] = tmp.sum(0) # Sum of row array elements
    ...
    def __init__(self, V, rank, win=1, circular=False, **kwargs):
        ...
        alphaW, alphaZ, alphaH : float or appropriately shaped array
            ...
            **Note** that the prior is not parametrized in the
            standard way where the uninformative prior has alpha=1.
        """
        PLCA.__init__(self, V, rank, **kwargs) # Initialize the parent class
